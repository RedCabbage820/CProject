...
#include <string.h>
//구조체(struct)
//여러 자료형의 변수들을 하나로 묶어서 새로운 자료형으로 만드는 것을 의미합니다.(복합 자료형)
//사용자의 편의대로 묶어서 하나의 형태로 표현하는 방식이기에 사용자 정의타입이라고도 부릅니다.
//데이터의 구조화 작업을 진행할 때 사용하는 데이터입니다.
//구조화 : 연결되어 하나의 덩어리로써 처리하는 방식, 비슷한 개념으로는 record(c#), Object(java, c++)
...
int main()
{
	...
	//구조체변수명.변수명 = 값;을 통해 수정 가능합니다.
	s2.number = 2;
	strcpy(s2.name, "Nero");
	//문자열은 자체 수정이 불가능 하므로, strcpy() 함수를 통해 수정합니다.
	s2.group = 'B';
	...
	printf("이름 : ");
	scanf("%s", s3.name);
	getchar(); //문자열 다음에 문자를 받고 있으니 getchar()를 통해 버퍼 키우기
	printf("그룹 : ");
	scanf("%c", &s3.group);
