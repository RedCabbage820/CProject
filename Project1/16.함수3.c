...
//3. 가변 인자의 경우는 전달받은 값을 알 수가 없습니다. 그래서 처음의 받는 값은 보통 받은 값의 개수를
//	 알 수 있는 데이터를 전달해줍니다.
//4. va_list는 전달받은 가변인자를 저장할 데이터입니다.
//5. va_start(arg, count)는 가변 인자에 대한 처리를 진행합니다.
...
//2. 배열을 받는 경우
//배열을 전달할 경우에는 C언어가 자동적으로 베열의 길이를 확인할 수 없기 때문에
//따로 배열의 길이는 작성해줘야합니다.
void printArray(int arr[], in size)
{
	for (int i = 0l; i < int size; i++)
	{
		printf("%d ", arr[i]);
	}
	printf("\n");
