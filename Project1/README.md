...
|목차|이동|
|----|----|
|1.깃허브|[1. 깃허브 데스크톱](#깃허브-데스크톱)|
|ㅡㅡㅡ|ㅡㅡㅡ|
|2.코드|[2. 코드 기본 구성](#코드-기본-구성)
| |[2-1. printf](#printf)|
| |[2-2. 서식 지정자](#서식-지정자)|
|ㅡㅡㅡ|ㅡㅡㅡ|
|3.변수|[3. 변수](변수)|
|ㅡㅡㅡ|ㅡㅡㅡ|
|4.자료형|[4. 자료형](#자료형)|
| |[4-1. 대표적인 자료형](#대표적인-자료형)|
|ㅡㅡㅡ|ㅡㅡㅡ|
|5.변수에 대한 선언|[5. 변수에 대한 선언](#변수에-대한-선언)|
|ㅡㅡㅡ|ㅡㅡㅡ|
|6.입력|[6. 입력](#입력)|
| |[6-1.특수코드](#특수코드)|
|ㅡㅡㅡ|ㅡㅡㅡ|
...
int main()
{
	...
	//C언어이ㅔ서 10진수 이외의 8진수나 16진수를 지정할 수 있습니다.
...
int main()
{
	...
	scanf_s("%d", &number);
	//Visual Studio에서 사용하는 경우라면 scanf를 통해 오류를 막을 수 있습니다.
	printf("number : %d\n", number);

	//getchar()
	//키보드로부터 하나의 문자를 읽어내는 함수
	//이 기능은 \n에 버퍼를 비우는 용도로도 사용합니다.
	//입력 버퍼(buffer) : C언어에서 입력을 진행할 경우 값을 넘기는게 아니라
	...
	char key;
	printf("키를 하나 입력해 주세요 >> ");
	key = getchar();
	putchar(key);

	//gets(), puts()
	//문자열에 대한 입력과 출력을 진행할 때 편하게 사용할 수 있습니다.
	//문자열(문장)이란 char 형태의 데이터를 묶음으로 표현한 것을 의미합니다.
	...
	//주의사항 : C언어에서 한글입력하는 경우 한글 한 단어에 2칸을 소모합니다.
	...
	char item[10];
	printf("아이템 이름을 입력해주세요 >> ");
	scanf("%s", item);
	//문자열을 받을 경우에는 item 앞에 &를 붙이지 않아도 됩니다.
	//%s는 문자열을 받는 지정자입니다.
	printf("아이템의 이름 : %s 입니다.", item);
