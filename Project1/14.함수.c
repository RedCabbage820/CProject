...
//4. #inlcude를 통해 라이브러리에서 제공해주는 헤더 파일을  코드에 연결해야 쓸 수 있는 라이브러리 함수가 존재합니다.
...
//1. 반환 타입의 경우 해당 함수를 실행한 뒤 넘겨줄 결과값의 형태룰 표시합니다.
...
//4. return코드를 실행하면, 함수가 종료됩니다. 추가적으로 return 다음에 값이 작성되어있는 경우 함수를 호출한 위치에 그 값을 전달합니다.

int add(int a, int b) //매개변수는 a, b
{
	int result = a + b;
	return result; //add함수의 결과물
}
int sub(int a, int b)
{
