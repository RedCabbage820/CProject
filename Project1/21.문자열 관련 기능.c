...
int main()
{
	...
	printf("array1의 문자열의 길이 = %ul\n", strlen(array1));
	printf("array1의 문자열의 길이 = %ul (sizeof)\n", sizeof(array1));
	//% : 서식 지정자
	//l : long(정수)
	//u : unsigned(부호가 따로 없습니다.(음수가 아님을 표현)
	printf("array2의 문자열의 길이 = %lu\n", strlen(array2));
	printf("array2의 문자열의 길이 = %ul (sizeof)\n", sizeof(array2));
	...
	//함수 3. strncpy()
	//strncpy(바꿀 문장, 바꾸려는 문장, 개수);를 통해 문자열을 바구려는 문장 중 일부의 값만 가져오는게 가능합니다.

	char sample3[] = "Hello Everyone";
	char sample4[20];
	strcpy(sample4, sampl3, 5); //비어있는 값에다 복사해줄 경우 이상한 값이 들어가게 됨.
	sample4[5] = '\0' //마지막 부분을 \0으로 처리하게 됩니다.
	printf("%s", sample4);

	//함수 4. strcat()
	//strcat(문자열1, 문자열2)를 통해 문자열1에 문자열2를 합칠 수 있습니다.
	char sample5[20] = "C언어";
	char sample6[] = "프로그래밍";
