...
//
////return의 기능
////1. 함수를 종료하고, 함수를 호출한 위치로 이동시킵니다.(return이 없는 경라면 함수의 명령이 끝나면 자동으로 종료
////2. return 다음에 값이 작성된다면, 함수를 호출한 위치에 해당 값을 전달합니다.
////3. C/C++의 return값은 1개만 전달이 가능합니다.
//
////함수의 인수, 매개변수
...
////						3. 함수의 매개변수 또한, 일종의 지역 변수입니다.(함수 호출 시 값을 받고, 변수로 사용되며 종료 시 삭제되기 때문)
////3. 정적 변수(Static) : 프로그램이 종료되기 전까지 소멸하지 않는 변수
////						1. static 키워드가 붙은 변수를 의미합니다. ex) static in count;
////						2. 정적 변수의 초기화는 1번만 진행합니다.
////						3. 초기화하눈 값은 반드시 상수여야 합니다.
////						4. 함수 내에서 만들어도, 해당 변수는 사라지지 않고 유지됩니다.(정적 지연 변수:static local)
...
////		 프로그랩이 시작하고 종료하는 순간까지 메모리에 남아있게 됩니다.
//
////2. Data 영역
...
////		 C언어에서는 free()을 통해 할당을 해제하며, C++에서는 delete를 통해 할당을 해제합니다.
...
////		 스택 영역에 저장되는 함수의 호출 정볼를 스택 프레임(stack Frame)이라고 부릅니다.
...
////		 ---> 높은 주소(아래)로부터 낮은 주소(위)로 데이터가 쌓이는형태
//
////오버플로(OverFlow) : 한정되어있는 메모리 공간이 부족해 메모리 안에 있는 네이터가 넘쳐 흐르는현상
////문제되는 이유 : 데이터가 넘치면 상대 공간을 침범하는 일이 발생할 수 있고, 이는 데이터를 데이터를 제대로 불러올 수 없는 상황까지 이어집니다.
//
//
//int a = 5;
//
...
//	GlobalF();
//	cout << ("main/global") << a << endl;
//	int a = 10; //전역 변수 a가 있는 상태에서 지역변수 a가 새로 만들어진다면, 함수 내에서는 지역 변수를 우선 처리합니다.
//	GlobalF();
//	cout << ("main/local") << a << endl;//현 시점에서 main의 a는 지역변수이기 때문에 GlobalF에 의해 전역변수 a의 값이 올라도, main의 a랑은 전혀 상관없는 값이 됩니다.
//	ParameterF(a);
//	cout << ("main/local") << a << endl;
//	//매개변수로 a의 "값"을 전달합니다. 그리고 ParameterF에서 그 값을 기반으로 매개변수 a를 할당합니다.
//	//그 매개변수 a와 main의 지역변수 a는 서로 다른 데이터입니다.
//	//call by value(값에 의한 호출)이라고 부르며, 함수가 종료되도 변경된 내용이 적용되지 않습니다.
//	//해결 방안 : call by reference(참조에 의한 전달) : 매개변수를 포인터로 받거나, 참조자로
//	staticF();
//	cout << ("main/local") << a << endl;
//
//	cout << "5번 연속 출력" << endl;
