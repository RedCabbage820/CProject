...
//using namespace std;
//
////new와 delete는 코드 내에서 데이터를 만들고, 삭제할 수 있는 동적 할당을 위한 코드
////일반적으로는 사용자가 데이터를 만들어 프로그램에 포함되지만(정적 할당)
////동적 할당으로 코드를 짜면, 실행 과정에서 만들어지기에 프로그램 용량에 포함이 안되고,
////반납을 통해 필요없을 때 용량을 비울 수 있게 되면서 유연하게 프로그램을 구현할 수 있습니다.
//
////C언어에서의 동적 할당 방법
////1. #include <stdlib.h>
////2. 자료형* 변수명 = malloc(sizeof(자료형) * 크기); <-를 통해 변수 목적 할당(new)
...
////2. delete(변수명);
//
////동적 할당은 기본적으로, 포인터 변수에 메모리를 할당해주는 행위입니다.
//
//int main()
//{
//	//1. 생성 후 값 설정까지
